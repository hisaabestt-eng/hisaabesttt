...
export async function getCompanies() {
  const { rows } = await pool.query(
    "SELECT comp_id, company_name, status FROM companies ORDER BY (status = 'Active') DESC, company_name"
  );
  return rows;
}
...
// Includes Inactive clients too, same reasoning as getCompanies() above.
export async function getClients() {
  const { rows } = await pool.query(`
    SELECT DISTINCT cl.client_id, cl.client_name, cl.status, r.comp_id
    FROM clients cl
      ...
      SELECT json_agg(json_build_object(
        ...
        'status', CASE
          ...
          WHEN i2.submission_status IS NOT NULL AND i2.submission_status <> 'Not Submitted' THEN
            CASE WHEN i2.scheduled_payment_date IS NOT NULL THEN 'Scheduled' ELSE 'In Progress' END
          ELSE 'Raised Not Submitted Yet'
        END
