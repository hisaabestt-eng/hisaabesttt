import { pool } from "./db";
import { applyDateRangeFilter, applyEnteredOnFilter } from "./dates";
...
const BASE_CTE = `
  WITH base AS (
    SELECT
      ...
      (
        SELECT string_agg(elem->>'invoice_no', ' ')
        FROM json_array_elements(
          COALESCE(CASE WHEN po.po_id IS NOT NULL THEN inv_list.invoices ELSE direct_inv_list.invoices END, '[]'::json)
        ) elem
      ) AS invoice_nos_text,
      ...
      CASE
        ...
        -- end — nothing further down the chain (estimate/PO/invoice) can be
        -- created for it, so that state should win over "X Pending" even
        -- when nothing's been raised yet, checked from the earliest stage
        -- down so an archived record doesn't get overridden by an archived
        -- estimate underneath it, etc.
        WHEN trim(lower(r.overall_status)) LIKE '%canc%' THEN 'Record Cancelled'
        WHEN r.is_archived OR trim(lower(r.overall_status)) LIKE '%archiv%' THEN 'Record Archived'
    ...
    LEFT JOIN LATERAL (
      SELECT
        ...
        -- alongside a paid one could never reach "Paid".
...
export async function getRecordsOverview({ compId, clientId, search, progress, year, yearType, from, to, enteredOn }) {
  const conditions = [];
  const params = [];
  ...
  applyDateRangeFilter(conditions, params, "estimate_date", { from, to, year, yearType });
  ...
  for (const word of searchWords) {
    ...
    const p = `$${params.length}`;
    conditions.push(
      `(record_id ILIKE ${p} OR estimate_description ILIKE ${p} OR client_name ILIKE ${p} OR est_no ILIKE ${p} OR po_no ILIKE ${p} OR invoice_nos_text ILIKE ${p} OR status ILIKE ${p} OR invoice_statuses_text ILIKE ${p} OR tags ILIKE ${p} OR estimate_amount::text ILIKE ${p})`
    );
  }
