...
// invoices (partial billing) — once there's invoice data to point to, the
// Amount column should total what those invoices actually bill (which can
// differ from the record's own estimate/PO amount once GST/TDS or partial
// invoicing enter the picture), the same way the Status badge above already
// prefers each invoice's own progress over the record-level aggregate.
// Sums invoice_amount (the pre-GST/TDS base figure) to match the "Invoice
// Amount" column on the Invoices page — invoice_total is the post-tax total.
export function mainRowAmount(row) {
  const invoices = row.invoices || [];
  if (invoices.length === 0) return Number(row.estimate_amount) || 0;
  return invoices.reduce((sum, inv) => sum + (Number(inv.invoice_amount) || 0), 0);
}
