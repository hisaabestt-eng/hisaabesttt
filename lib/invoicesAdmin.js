...
import { toDateString, applyDateRangeFilter, applyEnteredOnFilter } from "./dates";

export async function listInvoices({ compId, clientId, search, progress, year, yearType, from, to, enteredOn }) {
  ...
  applyDateRangeFilter(conditions, params, "invoice_date", { from, to, year, yearType });
  ...
  const { rows } = await pool.query(
    ...
    WITH base AS (
      SELECT
        i.inv_id, i.invoice_no, i.invoice_date, i.created_at, i.description,
...
export async function getDetailedInvoices({ compId, clientId, search, year, yearType, lifecycle, from, to, enteredOn }) {
  ...
  applyDateRangeFilter(conditions, params, "i.invoice_date", { from, to, year, yearType });
  applyEnteredOnFilter(conditions, params, "i.created_at", enteredOn);
...
export async function getPOsForPicker(compId, clientId) {
  ...
  const { rows } = await pool.query(
    ...
    WITH picker AS (
      ...
      WHERE r.comp_id = $1
        -- An archived/cancelled PO is a dead end — no invoice should be
        -- creatable for it.
        AND NOT po.is_archived
        AND trim(lower(po.status)) NOT LIKE '%archiv%'
        AND trim(lower(po.status)) NOT LIKE '%canc%'
        ${clientFilter}
    )
    SELECT * FROM picker
    WHERE balance > 0.01
  ...
  const { rows } = await pool.query(
    ...
    WITH picker AS (
      ...
      WHERE r.comp_id = $1
        ...
        -- An archived/cancelled estimate is a dead end — no invoice should
        -- be creatable straight off it either.
        AND NOT e.is_archived
        AND trim(lower(e.status)) NOT LIKE '%archiv%'
        AND trim(lower(e.status)) NOT LIKE '%canc%'
        ${clientFilter}
    )
    SELECT * FROM picker
    WHERE balance > 0.01
...
export async function updateInvoice(
  ...
  const { rows } = await pool.query(
    `SELECT i.invoice_no, i.po_no, i.est_id, i.status, i.is_archived,
  ...
  // A dead invoice (Cancelled/Rejected/Archived) never had to fit inside
  // the PO/Estimate's remaining balance — it isn't real financial
  // commitment. Without this exemption, an already-tight balance (common,
  // since a cancelled invoice's own amount is excluded from "already
  // invoiced" elsewhere) could block editing it at all, including
  // unrelated edits like attaching a reference document.
  const statusText = (rows[0].status || "").trim().toLowerCase();
  const isDeadEnd =
    statusText.includes("canc") || statusText.includes("reject") || rows[0].is_archived;
  ...
  if (hasPayment) {
    ...
    if (!isDeadEnd && Number(invoiceAmount) > remaining + 0.01) {
      throw new Error(
