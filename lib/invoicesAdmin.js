...
export async function listInvoices({ compId, clientId, search, progress, year, yearType }) {
  ...
  for (const word of words) {
    ...
    conditions.push(
      `(description ILIKE ${p} OR client_name ILIKE ${p} OR invoice_no ILIKE ${p} OR po_no ILIKE ${p} OR status ILIKE ${p} OR lifecycle ILIKE ${p} OR invoice_amount::text ILIKE ${p} OR invoice_total::text ILIKE ${p})`
    );
  }
  ...
  const { rows } = await pool.query(
    ...
    WITH base AS (
      SELECT
        ...
        i.scheduled_payment_date, i.submission_status, i.submission_date, i.custom_status,
        ...
        CASE
          ...
          WHEN i.submission_status IS NOT NULL AND i.submission_status != 'Not Submitted' THEN
            CASE WHEN i.scheduled_payment_date IS NOT NULL THEN 'Scheduled' ELSE 'In Progress' END
...
export async function getDetailedInvoices({ compId, clientId, search, year, yearType, lifecycle }) {
  ...
  for (const word of words) {
    ...
    const p = `$${params.length}`;
    conditions.push(
      `(i.description ILIKE ${p} OR cl.client_name ILIKE ${p} OR i.invoice_no ILIKE ${p} OR i.status ILIKE ${p} OR i.remarks ILIKE ${p} OR i.invoice_amount::text ILIKE ${p} OR i.invoice_total::text ILIKE ${p})`
    );
  }
...
export async function invoiceNoExists(invoiceNo) {
  const { rows } = await pool.query("SELECT 1 FROM invoices WHERE invoice_no = $1 LIMIT 1", [invoiceNo]);
...
export async function updateInvoice(
  ...
  await pool.query(
    `UPDATE invoices SET
       scheduled_payment_date = $1, submission_status = $2, submission_date = $3, updated_at = now()
     WHERE inv_id = $4`,
    [scheduledPaymentDate || null, submissionStatus || "Not Submitted", submissionDate || null, invId]
  );
...
export async function updatePaymentProgress(invId, { scheduledPaymentDate, rejected }) {
  const { rows } = await pool.query(
  ...
  if (rejected) {
    await pool.query(
      "UPDATE invoices SET scheduled_payment_date = NULL, status = 'Rejected', updated_at = now() WHERE inv_id = $1",
      [invId]
    );
    return;
  }
  await pool.query(
    "UPDATE invoices SET scheduled_payment_date = $1, updated_at = now() WHERE inv_id = $2",
    [scheduledPaymentDate || null, invId]
  );
}
