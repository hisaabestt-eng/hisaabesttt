...
// Allocations themselves aren't editable here — correcting which invoices
// got paid means deleting and re-adding the payment, since reallocating
// safely needs more validation than a simple field edit. The amount itself
// is fine to correct in place (e.g. a typo when it was first recorded), as
// long as it doesn't drop below what's already been allocated to invoices.
export async function updatePayment(pyId, { paymentDate, remarks, amountReceived }) {
  const { rows } = await pool.query(
    `SELECT COALESCE(sum(allocated_amount), 0) AS allocated FROM payment_allocations WHERE payment_id = $1`,
    [pyId]
  );
  const alreadyAllocated = Number(rows[0].allocated);
  if (Number(amountReceived) < alreadyAllocated - 0.01) {
    throw new Error(
      `Amount can't be less than what's already allocated to invoices (₹${alreadyAllocated.toFixed(2)}).`
    );
  }
