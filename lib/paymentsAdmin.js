...
export async function listPayments({ compId, clientId, search, year, yearType, from, to, enteredOn }) {
  ...
  applyDateRangeFilter(conditions, params, "p.payment_date", { from, to, year, yearType });
  applyEnteredOnFilter(conditions, params, "p.created_at", enteredOn);
...
export async function listInvoiceSummaries({ compId, clientId, search, progress, year, yearType, from, to, enteredOn }) {
  ...
  for (const word of words) {
    ...
    const p = `$${params.length}`;
    conditions.push(
      `(record_id ILIKE ${p} OR invoice_no ILIKE ${p} OR po_no ILIKE ${p} OR est_no ILIKE ${p} OR client_name ILIKE ${p} OR invoice_total::text ILIKE ${p})`
    );
  }
  ...
  const { rows } = await pool.query(
    ...
    base AS (
      SELECT
        i.inv_id, i.invoice_no, i.invoice_total, i.invoice_date, i.created_at, i.custom_status,
        ...
        r.record_id, i.po_no, est.est_no,
        r.comp_id, r.client_id, cl.client_name,
...
export async function getOutstandingInvoices(compId, clientId) {
  const { rows } = await pool.query(
    ...
    WHERE r.comp_id = $1 AND r.client_id = $2 AND i.is_archived = false
      ...
      AND trim(lower(i.status)) NOT LIKE '%reject%'
...
export async function allocatePayment(pyId, { allocationDate, allocations }) {
  ...
  for (const a of cleanAllocations) {
    const { rows } = await pool.query(
      `SELECT i.invoice_total, i.record_id, i.status, i.is_archived, r.comp_id, r.client_id,
    ...
    // Same dead-end rule as everywhere else: a rejected/cancelled/archived
    // invoice shouldn't receive a new payment allocation, even if a request
    // for it slips past the picker somehow.
    const statusText = (inv.status || "").trim().toLowerCase();
    if (statusText.includes("reject")) throw new Error(`Invoice ${a.invoiceNo} was rejected — can't allocate to it.`);
    if (statusText.includes("canc")) throw new Error(`Invoice ${a.invoiceNo} was cancelled — can't allocate to it.`);
    if (inv.is_archived || statusText.includes("archiv")) {
      throw new Error(`Invoice ${a.invoiceNo} is archived — can't allocate to it.`);
    }
