...
// For invoice numbers that should open just their own document (no chain
// popup) — Records/Estimates/Purchase Orders pages' nested invoice tables.
export async function getInvoiceDocument(invoiceNo) {
  const { rows } = await pool.query(
    `
    SELECT d.doc_id, d.module, d.module_id, d.file_name, d.external_url
    FROM invoices i
