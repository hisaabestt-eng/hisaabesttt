...
// Scoped to the client, not just the company — each client has its own
// numbering, so the same Estimate No showing up under a different client
// (even within the same company) isn't a real duplicate.
export async function estNoExists(estNo, compId, clientId) {
  const { rows } = await pool.query(
    `SELECT 1 FROM estimates e
     JOIN records r ON r.record_id = e.record_id
     WHERE e.est_no = $1 AND r.comp_id = $2 AND r.client_id = $3 LIMIT 1`,
    [estNo, compId, clientId]
  );
  return rows.length > 0;
}

// Only the trailing digits go up by one (e.g. "ES-054" -> "ES-055") — the
// prefix before them is left untouched, since that's not something a plain
// +1 can safely guess at. Fallback for estimates that don't use the
// FY-tagged format below (e.g. older data from before it was adopted).
...
// India's financial year runs 1 April - 31 March, so Jan-Mar still belongs
// to the FY that started the previous April.
function currentFinancialYearTag() {
  const now = new Date();
...
// client, not per company — each client has its own numbering, so one
// client's running count (or financial-year rollover) never leaks into
// another's suggestion.
//
// For clients using the FY-tagged format, the running number resets to 001
// each financial year rather than climbing forever — continuing last year's
// count into this year would be wrong even though, numerically, it's still
// just "the highest number so far". Which FY is "current" is based on
// today's date, not on any estimate's own date, so it matches the FY a user
// creating an estimate right now is actually in.
export async function getSuggestedEstNosByClient(compId) {
  ...
  const currentFY = currentFinancialYearTag();
  ...
  for (const [clientId, estNos] of byClient) {
    ...
    if (fyMatches.length > 0) {
      const sameFY = fyMatches.filter((m) => `${m[2]}/${m[3]}` === currentFY);
      if (sameFY.length > 0) {
        // Continue this FY's own sequence: highest number so far + 1.
        const best = sameFY.reduce((a, b) => (Number(a[4]) >= Number(b[4]) ? a : b));
        const [, prefix, , , digits] = best;
        const next = String(Number(digits) + 1).padStart(digits.length, "0");
        result[clientId] = `${prefix}${currentFY}-${next}`;
      } else {
        // No estimate yet this FY for this client — start over at 001,
        // matching the prefix/padding style from their most recent FY.
        const mostRecent = fyMatches.reduce((a, b) => (`${a[2]}${a[3]}` >= `${b[2]}${b[3]}` ? a : b));
        const [, prefix, , , digits] = mostRecent;
        result[clientId] = `${prefix}${currentFY}-${"1".padStart(digits.length, "0")}`;
      }
    } else {
