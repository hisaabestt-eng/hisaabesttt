...
// numbering scheme as the highest-numbered estimate for that client).
// Scoped per client, not per company — each client has its own numbering,
// so one client's running count shouldn't leak into another's suggestion.
// Based on the highest trailing number actually in use, not whichever
// estimate happens to have been created most recently — estimates are
// often entered out of order (e.g. backfilling an older record after
// already raising a later-numbered one), so "most recently created" and
// "highest number so far" are frequently different rows. Returns every
// client's suggestion in one query rather than one query per client.
export async function getSuggestedEstNosByClient(compId) {
  const { rows } = await pool.query(
    `SELECT DISTINCT ON (r.client_id) r.client_id, e.est_no
     FROM estimates e
     JOIN records r ON r.record_id = e.record_id
     WHERE r.comp_id = $1 AND e.est_no ~ '\\d+$'
     ORDER BY r.client_id, (regexp_match(e.est_no, '(\\d+)$'))[1]::int DESC`,
    [compId]
  );
  const result = {};
  for (const row of rows) {
    result[row.client_id] = incrementTrailingNumber(row.est_no) || "";
  }
  return result;
}
