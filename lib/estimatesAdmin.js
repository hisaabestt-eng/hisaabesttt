...
export async function listEstimates({ compId, clientId, search, progress, year, yearType, from, to, enteredOn }) {
  ...
  const { rows } = await pool.query(
    ...
    WITH base AS (
      SELECT
        e.est_id, e.est_no, e.estimate_date, e.created_at, e.description, e.amount, e.tags, e.is_archived, e.custom_status,
      ...
      LEFT JOIN LATERAL (
        SELECT
          ...
          -- it in this sum meant a fully-paid estimate with one cancelled
          -- invoice alongside a paid one could never reach "Paid".
