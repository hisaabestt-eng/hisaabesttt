...
// numbering scheme as the highest-numbered estimate for this company).
...
// already raising a later-numbered one), so "most recently created"
// and "highest number so far" are frequently different rows.
export async function getSuggestedEstNo(compId) {
  const { rows } = await pool.query(
    `SELECT e.est_no
     ...
     ORDER BY (regexp_match(e.est_no, '(\\d+)$'))[1]::int DESC
     LIMIT 1`,
    [compId]
  );
  if (!rows[0]) return "";
