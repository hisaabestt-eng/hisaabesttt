...
// fully editable, so this only saves typing on the common case. Scoped to
// the company as a whole (same as Invoice No/PO No) — one continuous
// sequence shared across every client, not a separate one per client.
//
// For the FY-tagged format, the running number resets to 001 each financial
// year rather than climbing forever — continuing last year's count into
// this year would be wrong even though, numerically, it's still just "the
// highest number so far". Which FY is "current" is based on today's date,
// not on any estimate's own date, so it matches the FY a user creating an
// estimate right now is actually in.
export async function getSuggestedEstNo(compId) {
  const { rows } = await pool.query(
    `SELECT e.est_no FROM estimates e JOIN records r ON r.record_id = e.record_id WHERE r.comp_id = $1`,
    [compId]
  );
  const estNos = rows.map((r) => r.est_no);
  ...
  if (fyMatches.length > 0) {
    ...
    if (sameFY.length > 0) {
      ...
      return `${prefix}${currentFY}-${next}`;
    }
    // No estimate yet this FY — start over at 001, matching the
    // prefix/padding style from the most recent FY in use.
    const mostRecent = fyMatches.reduce((a, b) => (`${a[2]}${a[3]}` >= `${b[2]}${b[3]}` ? a : b));
    const [, prefix, , , digits] = mostRecent;
    return `${prefix}${currentFY}-${"1".padStart(digits.length, "0")}`;
  }

  // Nobody's used the FY-tagged format yet — fall back to plain "increment
  // the highest trailing number in use" behavior.
  let best = null;
