...
// Accepts a JS Date (from Excel's own date cells) or a plain string in
// YYYY-MM-DD or DD/MM/YYYY (or DD-MM-YYYY) form. Rejects anything that isn't
// a real calendar date (e.g. 31/02/2026) rather than silently rounding it.
...
export async function listRecords({ compId, clientId, search, progress, year, yearType }) {
  ...
  for (const word of words) {
    ...
    conditions.push(
      `(description ILIKE ${p} OR client_name ILIKE ${p} OR status ILIKE ${p} OR lifecycle ILIKE ${p} OR amount::text ILIKE ${p})`
    );
  }
