...
import { parseFlexibleDate, toDateString, applyDateRangeFilter, applyEnteredOnFilter } from "./dates";
...
const RECORDS_STATUS_CTE = `
  WITH base AS (
    SELECT
      r.record_id, r.record_date, r.created_at, r.description, r.amount, r.comp_id, r.is_archived, r.custom_status,
      ...
      CASE WHEN po.po_id IS NOT NULL THEN inv_stats.invoice_nos_text ELSE direct_inv.invoice_nos_text END
        AS invoice_nos_text,
      CASE
        -- A record/estimate/PO that's been archived or cancelled is a dead
        -- end — nothing further down the chain can be created for it, so
        -- that state should win over "X Pending" even when nothing's been
        -- raised yet, checked from the earliest stage down (mirrors the
        -- same cascading check in records.js's BASE_CTE for the Main page).
        WHEN e.est_id IS NULL THEN 'Estimate Pending'
        WHEN trim(lower(e.status)) LIKE '%canc%' THEN 'Estimate Cancelled'
        WHEN e.is_archived OR trim(lower(e.status)) LIKE '%archiv%' THEN 'Estimate Archived'
        WHEN po.po_id IS NOT NULL AND trim(lower(po.status)) LIKE '%canc%' THEN 'PO Cancelled'
        WHEN po.po_id IS NOT NULL AND (po.is_archived OR trim(lower(po.status)) LIKE '%archiv%') THEN 'PO Archived'
    ...
    LEFT JOIN LATERAL (
      SELECT
        ...
        -- A cancelled invoice isn't real outstanding liability — leaving it
        -- in this sum meant a fully-paid record with one cancelled invoice
        -- alongside a paid one could never reach "Paid" (paid amount could
        -- never catch up to a total that includes money nobody owes).
        sum(i.invoice_total) FILTER (WHERE trim(lower(i.status)) NOT LIKE '%canc%') AS total_invoice_amount,
...
// Powers the Record Detail page's Previous/Next buttons — same client, same
// order the Records tab shows by default (record_date, record_id both ASC),
// so "Next" moves the same direction as scrolling down that list. Doesn't
// account for whatever search/progress filter the user came in from, since
// that state doesn't travel with a plain record_id link — just the client's
// full chronological order.
export async function getAdjacentRecordIds(recordId, compId, clientId) {
  const { rows: currentRows } = await pool.query(
    "SELECT record_date FROM records WHERE record_id = $1",
    [recordId]
  );
  const current = currentRows[0];
  if (!current) return { prevId: null, nextId: null };

  const params = [compId, clientId, current.record_date, recordId];
  const [{ rows: prevRows }, { rows: nextRows }] = await Promise.all([
    pool.query(
      `SELECT record_id FROM records
       WHERE comp_id = $1 AND client_id = $2 AND (record_date, record_id) < ($3, $4)
    ...
    pool.query(
      `SELECT record_id FROM records
       WHERE comp_id = $1 AND client_id = $2 AND (record_date, record_id) > ($3, $4)
       ORDER BY record_date ASC, record_id ASC LIMIT 1`,
      params
    ),
  ]);
  return { prevId: prevRows[0]?.record_id || null, nextId: nextRows[0]?.record_id || null };
}

export async function listRecords({ compId, clientId, search, progress, year, yearType, from, to, enteredOn }) {
  ...
  applyDateRangeFilter(conditions, params, "record_date", { from, to, year, yearType });
  applyEnteredOnFilter(conditions, params, "created_at", enteredOn);
...
export async function getRecordsWithoutEstimate(compId, clientId) {
  ...
  const { rows } = await pool.query(
    ...
    WHERE r.comp_id = $1
      ...
      -- An archived/cancelled record is a dead end — no estimate should be
      -- creatable for it.
      AND NOT r.is_archived
      AND trim(lower(r.overall_status)) NOT LIKE '%archiv%'
      AND trim(lower(r.overall_status)) NOT LIKE '%canc%'
      ${clientFilter}
...
export async function updateRecord(recordId, { recordDate, description, amount, customStatus }) {
  const { rows } = await pool.query(
    `SELECT r.record_date AS old_record_date, e.est_id, e.estimate_date, po.po_id
     FROM records r
     LEFT JOIN estimates e ON e.record_id = r.record_id
     LEFT JOIN purchase_orders po ON po.estimate_id = e.est_id
     WHERE r.record_id = $1`,
  ...
  // The estimate's date only follows the record's if the two were still in
  // sync (never manually diverged) — otherwise a deliberate difference
  // (e.g. invoicing on a different date than the internal record) would get
  // silently overwritten every time the record itself is edited.
  const oldRecordDate = rows[0] ? toDateString(rows[0].old_record_date) : null;
  const estimateInSync =
    estId && oldRecordDate && rows[0].estimate_date && toDateString(rows[0].estimate_date) === oldRecordDate;
  ...
  if (estimateInSync) {
    await pool.query("UPDATE estimates SET estimate_date = $1, updated_at = now() WHERE est_id = $2", [
      recordDate,
      estId,
    ]);
  }
