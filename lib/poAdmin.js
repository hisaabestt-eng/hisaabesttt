...
export async function listPOs({ compId, clientId, search, progress, year, yearType }) {
  ...
  for (const word of words) {
    ...
    conditions.push(
      `(description ILIKE ${p} OR client_name ILIKE ${p} OR po_no ILIKE ${p} OR status ILIKE ${p} OR lifecycle ILIKE ${p} OR amount::text ILIKE ${p})`
    );
  }
  ...
  const { rows } = await pool.query(
    ...
    WITH base AS (
      ...
      LEFT JOIN LATERAL (
        ...
        -- with its own status) for the row's drill-down list.
...
export async function poNoExists(poNo) {
  const { rows } = await pool.query("SELECT 1 FROM purchase_orders WHERE po_no = $1 LIMIT 1", [poNo]);
