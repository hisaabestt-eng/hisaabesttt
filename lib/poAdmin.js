...
export async function listPOs({ compId, clientId, search, progress, year, yearType, from, to, enteredOn }) {
  ...
  applyDateRangeFilter(conditions, params, "po_date", { from, to, year, yearType });
  ...
  const { rows } = await pool.query(
    ...
    WITH base AS (
      SELECT
        po.po_id, po.po_no, po.po_date, po.created_at, po.description, po.amount, po.is_archived, po.custom_status,
        r.record_id, r.comp_id,
        cl.client_id, cl.client_name,
        e.est_id, e.est_no, e.tags AS estimate_tags, e.estimate_date, e.description AS estimate_description,
        inv.inv_id,
        d.doc_id, d.file_name, d.document_type, d.external_url,
        est_doc.doc_id AS estimate_doc_id, est_doc.file_name AS estimate_file_name, est_doc.external_url AS estimate_external_url,
      ...
      LEFT JOIN LATERAL (
        SELECT
          ...
          -- A cancelled invoice isn't real outstanding liability — leaving
          -- it in this sum meant a fully-paid PO with one cancelled invoice
...
export async function getEstimatesWithoutPO(compId, clientId) {
  ...
  const { rows } = await pool.query(
    ...
    WHERE r.comp_id = $1
      ...
      -- An archived/cancelled estimate is a dead end — no PO should be
      -- creatable for it.
