...
export function AddCompanyButton() {
  ...
  async function handleSubmit(e) {
    ...
    setError("");
    setSaving(true);
    const res = await fetch("/api/companies-admin", {
      method: "POST",
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            <h2 className="mb-3 text-base font-semibold text-gray-900 dark:text-gray-100">Add Company</h2>
...
export function EditCompanyButton({ compId, companyName }) {
  ...
  async function handleSubmit(e) {
    ...
    setError("");
    setSaving(true);
    const res = await fetch(`/api/companies-admin/${compId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      ...
      {open && (
        ...
        >
          <form
            ...
            className="w-full max-w-md rounded-lg bg-white dark:bg-gray-800 p-5 shadow-xl"
          >
            <h2 className="mb-3 text-base font-semibold text-gray-900 dark:text-gray-100">Edit Company</h2>
            {error && <p className="mb-2 text-sm text-red-600">{error}</p>}
...
export function CompanyStatusButton({ compId, status }) {
  const [saving, setSaving] = useState(false);
  const router = useRouter();
  ...
  async function handleToggle() {
    setSaving(true);
    const res = await fetch(`/api/companies-admin/${compId}/status`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export function DefaultCompanyButton({ compId, isDefault }) {
  const [saving, setSaving] = useState(false);
  const router = useRouter();

  async function handleClick() {
    setSaving(true);
    const res = await fetch(`/api/companies-admin/${compId}/default`, { method: "POST" });
    setSaving(false);
    if (!res.ok) {
      const data = await res.json();
      alert(data.error || "Could not set default company");
      return;
    }
    router.refresh();
  }
