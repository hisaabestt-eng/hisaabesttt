  ...
  if (estimatesWithoutPO.length === 0) {
    return (
      <span className="text-xs text-gray-400">
        No estimates without a PO — add one on the Estimates page first.
      </span>
    );
  }
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            <h2 className="mb-3 text-base font-semibold text-gray-900">Add Purchase Order</h2>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-500">PO No</label>
                <input
                  type="text"
                  value={poNo}
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-500">PO Date</label>
                <input
                  type="date"
                  value={poDate}
...
export function EditPOButton({ po }) {
  const [open, setOpen] = useState(false);
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch(`/api/po-admin/${po.po_id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ poNo, poDate, description, amount }),
    });
    const data = await res.json();
    if (!res.ok) {
      setSaving(false);
  ...
  async function handleArchiveToggle() {
    ...
    const res = await fetch(`/api/po-admin/${po.po_id}/archive`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ archived: !po.is_archived }),
    });
    const data = await res.json();
    setArchiving(false);
    if (!res.ok) {
      ...
      {open && (
        ...
        >
          <form
            ...
            className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg bg-white p-5 shadow-xl"
          >
            <h2 className="mb-3 text-base font-semibold text-gray-900">Edit Purchase Order</h2>
            {error && <p className="mb-2 text-sm text-red-600">{error}</p>}
            ...
            <div className="mt-4 flex items-center justify-between gap-2">
              ...
              >
                {po.is_archived ? "Unarchive" : "Archive"}
              </button>
